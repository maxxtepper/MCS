...
void mcs_rawInput_M(char* arg) {
	...
	if (std::getline(strm, run, ' ') && std::getline(strm, layers, ' ')) {};

	infileName = "run" + run + "_" + layers + "layers.txt";
	outfileName = "run" + run + "_" + layers + "layers.root";
	...
	std::cout << "inFile:  " << infileName << " attained\n";
	std::cout << "outFile: " << outfileName << "created\n";
