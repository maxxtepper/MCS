...
void errorAngle_Mall_Mean(char* arg){
...
//			TH1D *angle_histo = new TH1D("angle_histo", "Distribution of scattering angles", 80, -40, 40);
			...
			TH1D *aeCuts_histo = new TH1D("aeCuts_histo", "Distribution of scattering angles", 100, -40, 40);
...
*/			
			...
			Double_t scale[2][3] = {{1, 1}, {0.2, 4}};
			for (int i=0; i<2; i++) {
				magCuts[i] = normalGaus->GetParameter("magG")*scale[i][0];
				RMSCuts[i] = normalGaus->GetParameter("RMSG")*scale[i][1];
				std::cout << "magCuts[" << i << "] = " << magCuts[i] << std::endl;
				std::cout << "RMSCuts[" << i << "] = " << RMSCuts[i] << std::endl;
			}

			meanCuts = aeCuts_histo->GetMaximum();

			funcCuts->SetParameters(magCuts[0], meanCuts, RMSCuts[0], magCuts[1], RMSCuts[1]);	
//			funcCuts->SetParLimits(2, 0, 8);
			funcCuts->SetParLimits(0, 3, 8000);
			funcCuts->SetParLimits(3, 3, 8000);
			funcCuts->SetParLimits(1, -2, 2);
	//		funcCuts->SetParLimits(4, normalGaus->GetParameter("RMSG"), 20);
