...
//
//Follows a specific naming of the txt files---> run0_0layers.txt, for example, is the 0th run of 0 layers.
//Work with this naming scheme without modifying the file fetching and saving. 
//
//Example: mcs_rawInput("0 2 4") will generate the raw root files for all runs available for layers
//0, 2, and 4, based on the naming runX_Ylayers, where X is the run and Y is the layer.
//WARNING: If run 0, 1, and 3 exist, only runs 0 and 1 will be loaded, this code does not prevent gaps in naming or data.
...
void mcs_rawInput(char* arg) {
	...
	std::string outF, inF;
	std::string outPrefix, inPrefix;
	std::string layers, run;
	std::istringstream strm(arg);

	//NOTE: change these prefixes to desired destination of output files and proper location of input files
	...
	while (true) {	
		if (std::getline(strm, layers, ' ')) {
			...
			while (true) {
				...
				//if the file can not be opened, fail and move onto next argument (next layer)
				//This means it found no more runs with the nameing, so will move onto next
				//layer if one exists in arg
				if (!fin.fail()) {
					...
					//Occurs when no more files were found for the current number of layers
			...
			//Occurs when no more arguments are left to resolve (no more layers left)
